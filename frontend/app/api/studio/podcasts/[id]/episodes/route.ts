...
/**
 * GET /api/studio/podcasts/[id]/episodes — List episodes for a specific podcast.
 */
export async function GET(
  ...
  try {
    ...
    // Verify the user owns the podcast
    const podcastRecord = await db.query.podcast.findFirst({
      where: and(eq(podcast.id, podcastId), eq(podcast.userId, session.user.id)),
    });

    if (!podcastRecord) {
      return NextResponse.json({ error: "Podcast not found or unauthorized" }, { status: 404 });
    const episodes = await db
      .select()
      .from(episode)
      .where(and(eq(episode.podcastId, podcastId), eq(episode.userId, session.user.id)))
      .orderBy(desc(episode.createdAt));

    const mappedEpisodes = episodes.map((ep) => {
      const blobName = extractBlobNameFromUrl(ep.audioUrl);
      const audioUrl = blobName ? generateReadSasUrl(blobName) : ep.audioUrl;
      return { ...ep, audioUrl };
    });

    return NextResponse.json({ podcast: podcastRecord, episodes: mappedEpisodes });
  } catch (error) {
